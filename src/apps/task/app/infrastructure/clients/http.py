...
from app.exceptions.infrastructure import ClientHTTPException, ClientConnectionException
...
class HTTPXClient(BaseClient):
    ...
    async def get(self, url: str, **kwargs: Any) -> ClientResponse:
        try:
            ...
            raise ClientHTTPException(message=f"HTTP error {e.response.status_code}", url=url) from e

        except httpx.RequestError as e:
            logger.error("Connection error for URL %s: %s", url, str(e))
            raise ClientConnectionException(message="Connection failed", url=url) from e


    @override
    async def post(self, url: str, data: Any = None, **kwargs: Any) -> ClientResponse:
        return HTTPXClientResponseWrapper(await self._client.post(url, data=data, **kwargs))
